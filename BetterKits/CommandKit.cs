...
namespace BetterKits
{
    public class CommandKit : IRocketCommand
    {
        ...
        public void Execute(IRocketPlayer caller, string[] command)
        {
            ...
            if (!globalCooldown.Equals(default(KeyValuePair<string, DateTime>)) && !caller.IsAdmin)
            {
            ...
            if (!individualCooldown.Equals(default(KeyValuePair<string, DateTime>)) && !caller.IsAdmin)
            {
            ...
            foreach (KitItem item in kit.Items)
            {
                ...
                try
                {
                    if (!player.GiveItem(item.ItemId,(byte)item.Amount))
                    {
